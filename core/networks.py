...
class Dis(nn.Module):
    def __init__(self, hyperparameters):
        ...
        self.tripleAttention = TripletAttention()
    def forward(self, x, s, y, i):
        f = self.conv(x)
        f = self.tripleAttention(f)
...
class Extractors(nn.Module):  # reference相关的代码
    def __init__(self, hyperparameters):
        super().__init__()
...
class Mapper(nn.Module):
    def __init__(self, hyperparameters, num_attributes):
        ...
        pre_channels = hyperparameters['mappers']['pre_channels']
        self.PreLine = nn.Linear(hyperparameters['noise_dim'], pre_channels[0])

        self.pre_model1 = KAN(layers_hidden=[256,256,256], grid_size=5, spline_order=3,
                             scale_noise=0.1, scale_base=1.0, scale_spline=1.0,
                             base_activation=torch.nn.SiLU)

        self.pre_model = nn.Sequential(
            *[KAN(layers_hidden=[256,256,256], grid_size=5, spline_order=3,
                             scale_noise=0.1, scale_base=1.0, scale_spline=1.0,
                             base_activation=torch.nn.SiLU) for i in range(4)]
        )

        # 后置KAN模型，每个属性一个
        post_channels = hyperparameters['mappers']['post_channels']
        self.post_models = nn.ModuleList([
            nn.Sequential(
                *[KAN(layers_hidden=[256, 256, 256], grid_size=5, spline_order=3,
                      scale_noise=0.1, scale_base=1.0, scale_spline=1.0,
                      base_activation=torch.nn.SiLU) for i in range(4)],
                nn.Linear(hyperparameters['style_dim'], hyperparameters['style_dim'])
                # 假设final_output_dim为最终输出维度
            )
