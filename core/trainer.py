...
arcface = ArcfaceResnet50.load_from_checkpoint("", in_features=512,out_features=10177, s=64.0, m=0.50)
arcface.eval()
...
def get_feat(x):
    ...
    x = x.to(device)
    if hasattr(arcface, "forward_features"):
        feat = arcface.forward_features(x)  # [B,512]
    elif hasattr(arcface, "get_embedding"):
        feat = arcface.get_embedding(x)  # [B,512]
    else:
        feat = arcface(x, return_feat=True)
...
class HiSD(nn.Module):
    ...
    def gen_losses(self, x, y, i, j, j_trg):
        ...
        s = self.gen.extract(x, i) #参考引导，根据x提取出特征
        e_slf = self.gen.translate(e, s, i) #将参考引导的结果与e一起进行转换
        x_slf = self.gen.decode(e_slf)


        # cycle-translation path
        ## translate
        s_trg = self.gen.map(torch.randn(batch, self.noise_dim).cuda(), i, j_trg)#潜在引导，生成目标特征
        e_trg = self.gen.translate(e, s_trg, i) #潜在引导，将s与e一起生成目标e1
        x_trg = self.gen.decode(e_trg)
        ## cycle-back
        e_trg_rec = self.gen.encode(x_trg)
        s_trg_rec = self.gen.extract(x_trg, i) #根据第一轮生成的结果，提取参考引导
        e_cyc = self.gen.translate(e_trg_rec, s, i)
        x_cyc = self.gen.decode(e_cyc)

        feat1 = get_feat(x)
        feat2 = get_feat(x_trg)
        feat3 = get_feat(x_cyc)
        loss_id_1 = 1.0 - F.cosine_similarity(feat1, feat2, dim=1).mean()
        loss_id_2 = 1.0 - F.cosine_similarity(feat1, feat3, dim=1).mean()
        ...
        loss_gen_total = self.hyperparameters['adv_w'] * loss_gen_adv + \
                         ...
                         self.hyperparameters['id_w'] * loss_id
